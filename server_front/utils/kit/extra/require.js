steel.d("utils/kit/extra/require", [],function(require, exports, module) {
/**
 * 异步加载管理模块
 * @author ZhouJiequn | jiequn@staff.sina.com.cn
 * @example
 * 	//常规用法(推荐)
 * 	var rm = $.utils.kit.extra.require;
 * 	rm.register('xx', ['ex.kit.xx']);
 * 	var myFun = rm.bind('xx', function(a, b){});
 * 	myFun(a, b);
 * 
 */
STK.register('utils.kit.extra.require', function($){
//	$.log = function(str){
...
//	$.log('kit.extra.require 加载！！');
	...
	var getRealURL = function(url){
		if(typeof conf['urlRule'] === 'function'){
			...
			url = url.replace(/\./ig,'/');
			return conf['baseURL'] + conf['projectName']+'/js/' + url + '.js?version=' + conf['version'];
		}
	};
