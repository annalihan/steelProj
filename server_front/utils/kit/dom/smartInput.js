steel.d("utils/kit/dom/smartInput", ["utils/kit/dom/textSelection"],function(require, exports, module) {
/**
 * author Robin Young | yonglin@staff.sina.com.cn
 * 还未完善
 */
require("utils/kit/dom/textSelection");
STK.register('utils.kit.dom.smartInput', function($){
	return function(input, spec){
		var conf, that, lengthLimit, getValue,selection; //generally;
		var startLazyInput , stopLazyInput, onLazyInput, lazyInputTimer, lazyInputStats = 'stop', lazyInputCache,focusNotice,blurNotice; //lazyInput;
		...
		selection = $.utils.kit.dom.textSelection(input);
		...
		focusNotice = function(){
			...
			if(conf['currentClass'] != null){
				$.addClassName(input.parentNode, conf['currentClass']);
			}
		};
		...
		blurNotice = function(){
			...
			if(conf['currentClass'] != null){
				$.removeClassName(input.parentNode, conf['currentClass']);
			}
		};
		...
		$.addEvent(input, 'keydown', onCtrlEnter);
		...
		that.destroy = function(){
			...
			$.removeEvent(input, 'keydown',onEnter);
			$.removeEvent(input, 'keydown',onCtrlEnter);
