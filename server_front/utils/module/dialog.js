steel.d("utils/module/dialog", ["utils/module/layer"],function(require, exports, module) {
/**
 * 对话框
...
STK.register('utils.module.dialog', function($){
...
//	};
	...
	return function(template, spec){
		...
		var init = function(){
			...
			layer = $.utils.module.layer(template,conf);
			box = layer.getOuter();
		...
		that.setMiddle = function(){
			...
			if(window.FrameInner && window.FrameInner.outInfo){
				...
				//_top = ($CONFIG.scroll.scrollTop - $CONFIG.scroll.top) + ($CONFIG.scroll.height - dia.h)/2;
			...
			window.FrameInner && FrameInner.diaAutoHeight && FrameInner.diaAutoHeight(box);
			return that;
		};
