steel.d("utils/module/dragoptz", ["utils/kit/dom/parseDOM","utils/kit/dom/firstChild","utils/kit/dom/parentElementBy"],function(require, exports, module) {
...
STK.register('utils.module.dragoptz', function($) {
    ...
    return function(node,conf) {
        var argsCheck, parseDOM, initPlugins, bindDOM, bindCustEvt, bindListener, destroy, delegate, init, that = {}, timer;
        conf.spaceTag = conf.spaceTag ? conf.spaceTag : 'div';
        conf.containment = conf.containment ? conf.containment : false;
        conf.hasSpecialElType = conf.hasSpecialElType || false;//是否可作用于某个拖拽节点
        conf.dragIconActionType = conf.dragIconActionType || null;
        var lock = true;
        //+++ 变量定义区 ++++++++++++++++++
        var _this = {
            DOM : {}, //节点容器
            ...
            DeventFun : {
                ...
                controlOver: function(spec) {
                    lock = false;
                    // _this.dragFun.start(spec.evt);
                }, 
                controlOut: function(spec) {
                    lock = true;
                }
            },
            /**
        ...
        bindCustEvt = function() {
            ...
            if (conf.hasSpecialElType && conf.dragIconActionType){
                _this.objs.delegate.add(conf.dragIconActionType, 'mouseover', _this.DeventFun.controlOver);
                _this.objs.delegate.add(conf.dragIconActionType, 'mouseout', _this.DeventFun.controlOut);
                /*var target = $.sizzle('[action-type=' + conf.dragIconActionType + ']', opts.el.parentNode)[0];
