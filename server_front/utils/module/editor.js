steel.d("utils/module/editor", ["utils/kit/dom/parseDOM","utils/kit/extra/textareaUtils","utils/kit/extra/count"],function(require, exports, module) {
/**
* Editor组件
...
* 暂略
*/
//require("utils/module/editorPlugin/plugin");
require("utils/kit/dom/parseDOM");
require("utils/kit/extra/textareaUtils");
require("utils/kit/extra/count");

STK.register("utils.module.editor", function($) {
	...
	return function(node,options) {
		...
		var custFuncs = {
			...
			focus : function(start,len){
				if(typeof start != 'undefined'){
					$.utils.kit.extra.textareaUtils.setCursor(nodes.textEl,start,len);
				}else{
					var text = $.utils.kit.extra.textareaUtils.getSelectedText(nodes.textEl);
					if(!text || text.length == 0){
						var end = nodes.textEl.value.length;
						$.utils.kit.extra.textareaUtils.setCursor(nodes.textEl,end);		
					}
				}
			...
			count : function(){
				...
				return $.utils.kit.extra.count(str);
			}
		...
		var bindDOMFuns = {		
			...
			cacheCurPos : function(){
				//$.log($.getEvent());
				var selValue = $.utils.kit.extra.textareaUtils.getSelectedText(nodes.textEl);
				var slen = (selValue == '' || selValue == null) ? 0 : selValue.length;
				var start = $.utils.kit.extra.textareaUtils.getCursorPos(nodes.textEl);
		...
		var bindCustEvtFuns = {
			...
			textInput : function(_words,pos){
				...
				$.log('module.editor.sssssssssssss = '+pos);
				...
				//	pos = $.utils.kit.extra.textareaUtils.getCursorPos(nodes.textEl) || 0;
				//$.log('pos ',pos,len);
				//$.log(nodes.textEl.getAttribute('range'));
				$.utils.kit.extra.textareaUtils.unCoverInsertText(nodes.textEl,_words,{'rcs':cur[0],'rccl':cur[1]});
				//$.log(nodes.textEl.getAttribute('range'));
				bindDOMFuns.cacheCurPos();
				//$.log(nodes.textEl.getAttribute('range'));
				//$.utils.kit.extra.textareaUtils.insertText(nodes.textEl,_words,pos,len);
		...
		*/
		var parseDOM = function() {
			...
			/*for(var a in _nodelist){//todo if[a][0]就取[0]
				...
				nodes = $.utils.kit.dom.parseDOM(_nodelist);
