steel.d("utils/module/mask", ["utils/kit/dom/fix"],function(require, exports, module) {
/**
 * 遮罩工具
...
require("utils/kit/dom/fix");

STK.register("utils.module.mask", function($) {
	var maskNode,
		nodeRegList = [],
		domFix,
		maskInBody = false,
	...
	function initMask() {
		...
		maskInBody = true;
		domFix = $.utils.kit.dom.fix(maskNode, "lt");
		var _beforeFixFn = function () {
			var _winSize = $.core.util.winSize();
			maskNode.style.cssText = $.utils.kit.dom.cssText(maskNode.style.cssText)
